...
/* 
        ...
        return true;
          ...
          row--;  // comienza 1 posicion antes para que encaje en las coordenadas
        col--;  // comienza 1 posicion antes para que encaje en las coordenadas

        // 2. Verificar que las coordenadas esten en rango.
        if (row < 0 || row >= Board.tamaño - 1) return false;
        if (col < 0 || col >= Board.tamaño - 1) return false;
...
/* 
    ...
    // 3. Verificar si ya existe una pared vertical en cualquiera de los dos segmentos.
    if (board.vEdges[row][col] || board.vEdges[row+1][col]) return false;

    // 4. Colocar la pared vertical: marca 2 posiciones como true
    board.vEdges[row][col] = true;
    board.vEdges[row+1][col] = true;
